...
def hunt_main_text(data: dict) -> str:
    ...
    if sword:
        eq_block = (
            f'<blockquote>'
            f'{_tg(_E["sword"], "⚔️")} <b>Активный меч:</b> {_tg(sword["emoji_id"], "🗡")} <b>{sword["name"]}</b>\n'
